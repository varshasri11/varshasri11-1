<h1 align="center">Hi 👋, I'm Bantu Varsha Sri</h1>
<h3 align="center">Aspiring Data Scientist | AI & ML Enthusiast</h3>

<p align="center">
  <img src="https://media.giphy.com/media/hvRJCLFzcasrR4ia7z/giphy.gif" width="35">
</p>

---

## 🚀 About Me

🎓 **B.Tech CSE (Data Science)** | Malla Reddy Engineering College for Women  
💡 Passionate about **Data Science, AI, ML, DL, and Generative AI**  
🧠 Hands-on experience in **Machine Learning, NLP, CNNs, and LLM evaluation**  
📊 Strong foundation in **DSA, SQL, and model evaluation**

<img align="right" src="https://media.giphy.com/media/26tn33aiTi1jkl6H6/giphy.gif" width="280"/>

---

## 🔭 Currently Working On
- 🤖 AI & Generative AI  
- 📊 Data Science & Machine Learning Projects  
- 🧠 Prompt Engineering & LLM Evaluation  

---

## 🤖 Generative AI Focus
- 🔍 Evaluating GPT-based & open-source LLMs  
- 🧪 Prompt engineering & benchmarking  
- ⚠️ Hallucination & safety analysis  
- 🧠 NLP with BERT & embeddings  
- 📈 Model evaluation & UX improvement  

---

## 🌱 Currently Learning
- Machine Learning & Deep Learning  
- Model Optimization & Evaluation  
- Applied Data Science Workflows  

---

## 👯 Looking to Collaborate On
- ☕ Java  
- 🐍 Python  
- 🗄️ MySQL & SQL  
- 📊 Power BI  
- 🤖 ML & AI Projects  

---

## 💬 Ask Me About
- Java & Python  
- Data Science Projects  
- Machine Learning & AI Basics  

---

## 🧠 Internship Experience

### 🔹 R&D Intern – Ashna AI (May 2025 – Aug 2025)
- Benchmarked a **ChatGPT-like AI platform**
- Compared outputs with **GPT-based & open-source LLMs**
- Analyzed **hallucinations, safety compliance & UX**
- Contributed to **prompt refinement**

### 🔹 UI/UX Intern – Infosys Springboard (Feb 2025 – Apr 2025)
- Built **TeamSync**, a secure collaboration platform
- Implemented **JWT-based authentication**
- Designed intuitive UI using **Figma**

---

## 🚀 Projects

### 🧾 Emotion Recognition from Text
- Multi-label emotion classification using Twitter data
- Models: **SVM, Random Forest, BERT**
- Achieved **~80–85% F1-score with BERT**

### 🩺 Skin Lesion Classification using CNN
- CNN-based medical image classification
- Image preprocessing & augmentation
- Achieved **~75–80% accuracy**

---

## 🛠️ Languages & Tools

<p align="left">
  <img src="https://skillicons.dev/icons?i=java,python,html,css,js,mysql,github,vscode,figma,tensorflow" />
</p>

<p>
  <img src="https://img.shields.io/badge/NumPy-013243?style=for-the-badge&logo=numpy&logoColor=white"/>
  <img src="https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white"/>
  <img src="https://img.shields.io/badge/Scikit--Learn-F7931E?style=for-the-badge&logo=scikit-learn&logoColor=white"/>
  <img src="https://img.shields.io/badge/Seaborn-4C72B0?style=for-the-badge"/>
</p>

---

## 📊 GitHub Stats

<p align="center">
 <!-- <img
    src="https://github-readme-stats.vercel.app/api?username=varshasri11&show_icons=true&theme=tokyonight&hide_border=true"
    height="165"
    alt="GitHub stats"
  /> -->

  <img
    src="https://github-readme-streak-stats.herokuapp.com/?user=varshasri11&theme=tokyonight&hide_border=true"
    height="165"
    alt="GitHub streak stats"
  />
</p>


---

## 📜 Certifications
- Microsoft Azure AI – Edunet  
- Python Essentials – Cisco  
- Data Analytics & Visualization – Forage  
- Cloud Infrastructure & Generative AI – Oracle  
- Cambridge English – C1  
- Java Programming – Infosys Springboard  

---

## 🌍 Languages Known
- English  
- Telugu  
- Hindi  

---

## 🤝 Volunteering & Activities
- Technical workshops & hackathons  
- Volunteered at **TalTalks, T-Hub**

---

## 📫 Connect With Me

<p align="left">
  <a href="https://www.linkedin.com/in/bantuvarshasri" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
  <a href="https://mail.google.com/mail/?view=cm&fs=1&to=varshasribantu11@gmail.com" target="_blank">
    <img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>
  <a href="https://github.com/varshasri11" target="_blank">
    <img src="https://img.shields.io/badge/GitHub-000000?style=for-the-badge&logo=github&logoColor=white"/>
  </a>
</p>


---

<p align="center">
  <img src="https://media.tenor.com/8r0QMAcfdCAAAAAj/pink-thank-you.gif" width="280">
</p>

✨ *Always learning. Always building. Open to collaborations in AI & Data Science.*
